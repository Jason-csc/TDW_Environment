...
class MultiMagnebot(Controller):
    ...
    def getOBJ(self):
        for object_id,objt in self.objManager.objects_static.items():
            if "bowl" in objt.name or "box" in objt.name or "table" in objt.name:
                continue
            tmp = {}
            tmp["name"] = self.object_info[object_id]["scale"]+ " " + \
                        self.object_info[object_id]["color"] + " " + \
                        self.object_info[object_id]["name"]
            # update
            tmp["position"] = self.objManager.transforms[object_id].position
            tmp["reachable1"] = bool(tmp["position"][-1] < self.table_top["z"]+0.045)
            tmp["reachable2"] = bool(tmp["position"][-1] > self.table_top["z"]-0.045)
    ...
    def generateTask(self):
        #TODO: Read JSON file to get task
        task1 = [
                "Place the Medium Red Ball to the Blue Bowl",
                "Place the Medium Red Battery to the Black Bowl",
                "Place the Big Red Battery to the Brown Bowl"
                ]
        task2 = [
                "Place the Small Green Ball to the Blue Bowl",
                "Place the Medium Green Battery to the Orange Bowl",
                "Place the Big Red Pepper to the Orange Bowl",
                ]
        self.info["task"]["player1"] = task1
        self.info["task"]["player2"] = task2
    ...
    def setUp(self):
        ...
        bowl1_pos1 = {"x": table_top['x']+0.56, "y": table_top['y'], "z":table_top['z']-0.24}
        ...
        bowl1_pos2 = {"x": table_top['x']-0.56, "y": table_top['y'], "z":table_top['z']-0.24}
        ...
        bowl2_pos1 = {"x": table_top['x']+0.56, "y": table_top['y'], "z":table_top['z']+0.24}
        ...
        commands.extend([{"$type": "scale_object", "id": bowl2_id1, "scale_factor": {"x": 0.3, "y": 0.1, "z": 0.3}}])
        commands.extend([{"$type": "set_color", "color": {"r": 1, "g": 0.5, "b": 0, "a": 1}, "id": bowl2_id1}])


        bowl2_id2 = self.get_unique_id()
        bowl2_pos2 = {"x": table_top['x']-0.56, "y": table_top['y'], "z":table_top['z']+0.24}
        commands.extend(self.get_add_physics_object(model_name='box_tapered_white_mesh',
                                        library="models_core.json",
                                            position=bowl2_pos2,
                                            rotation={"x":0,"y":0,"z":0},
        ...
        commands.extend([{"$type": "scale_object", "id": bowl2_id2, "scale_factor": {"x": 0.3, "y": 0.1, "z": 0.3}}])
        ...
        share_pos1["x"] -= 0.1
        share_pos1["z"] += 0.041
        self.info["placePos1"].append({"name":"Black Bowl","pos":bowl1_pos1})
        self.info["placePos1"].append({"name":"Brown Bowl","pos":bowl1_pos2})
        self.info["placePos1"].append({"name":"sharePlace","pos":share_pos1})

        share_pos2 = table_top.copy()
        share_pos2["x"] -= 0.1
        share_pos2["z"] += -0.041
        self.info["placePos2"].append({"name":"Orange Bowl","pos":bowl2_pos1})
        self.info["placePos2"].append({"name":"Blue Bowl","pos":bowl2_pos2})
    ...
    def ObjectGenerator(self,commands,table_top):
        ...
        category_color = [
            ("Red",{"r": 1, "g": 0, "b": 0, "a": 1}),
            ("Green",{"r": 0, "g": 1, "b": 0.2, "a": 1}),
            ("Blue",{"r": 0.02, "g": 0.8, "b": 1.5, "a": 1})
        ]
        category_scale = [
        ...
        for i, obj in enumerate(objects):
            ...
            object_id = self.get_unique_id()
            self.object_info[object_id] = {"name":shape[0],"color":rgb[0],"scale":scale[0],"owner":owner}
